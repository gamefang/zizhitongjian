...
def main(str_csv, input_str):
    '''
    供外部調用的主函數
    '''
    dic_era = load_data_as_dict(str_csv)
    return view_query(dic_era, input_str.strip())
...
if __name__ == '__main__':
    ...
    from common import csv_loader # type: ignore
    path = os.path.abspath(os.path.join(current_dir, '../..', FP))
    result = main(csv_loader(path), QUERY_STR)
    print(result)